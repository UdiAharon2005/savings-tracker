...
if not st.session_state.logged_in:
    ...
    with tab_forecast:
        ...
        if deposits:
            ...
            rate_pessimistic = 0.04
            rate_optimistic = 0.08

            if st.button("Run Forecast"):
                months_forecast = 12 * years
                forecast_dates = [last_date + timedelta(days=30 * i) for i in range(1, months_forecast + 1)]

                pess = compute_growth(last_amount, monthly, years, rate_pessimistic)
                opti = compute_growth(last_amount, monthly, years, rate_optimistic)
            ...
            if st.session_state.forecast_triggered:
                ...
                ax3.plot(data["forecast_dates"], data["pess"], color="red", linestyle="--", label="Pessimistic Forecast")
                ax3.plot(data["forecast_dates"], data["opti"], color="green", linestyle="--", label="Optimistic Forecast")
                if data["forecast_dates"]:
                    ax3.annotate(f"₪{data['pess'][-1]:,.2f}", (data['forecast_dates'][-1], data['pess'][-1]),
                                 textcoords="offset points", xytext=(0,10), ha='center', fontsize=9, color='red')
                    ax3.annotate(f"₪{data['opti'][-1]:,.2f}", (data['forecast_dates'][-1], data['opti'][-1]),
